...
export class F1040Parser extends FormParser {
    ...
    looksLikeHeader(row) {
        ...
        const hasProperKeys = keys.some(k => k.toLowerCase() === 'variable' || k.toLowerCase() === 'value');
        if (hasProperKeys) {
    ...
    hasColumns(row, expectedColumns) {
        const keys = Object.keys(row).map(k => k.toLowerCase().trim());
        return expectedColumns.every(col => keys.some(k => k.includes(col) || col.includes(k)));
    }
    parseColumnFormat(rows, mapping, fieldValues, skipEmptyRows = true) {
        // Find the variable and value columns
        const firstRow = rows[0];
        if (!firstRow)
            return;
        const headers = Object.keys(firstRow);
        const variableCol = headers.find(h => h.toLowerCase().includes('variable') ||
            h.toLowerCase().includes('field') ||
            h.toLowerCase().includes('line'));
        const valueCol = headers.find(h => h.toLowerCase().includes('value') ||
    ...
    parseSimpleFormat(rows, mapping, fieldValues) {
