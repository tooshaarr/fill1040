...
export class F8949Parser extends FormParser {
    ...
    parse(rows, options) {
        if (rows.length === 0) {
            return this.createParseResult([], {}, this.createValidationResult(false, [{ field: 'sheet', message: 'No data found in sheet' }]));
        }
        // Parse transactions from Excel
        const transactions = this.parseTransactions(rows);
        if (transactions.length === 0) {
            return this.createParseResult([], {}, this.createValidationResult(false, [{ field: 'data', message: 'No valid transactions found' }]));
        }
    ...
    parseTransactions(rows) {
        ...
        for (const row of rows) {
            if (this.isEmpty(row[columnMap.quantity]))
                continue;
            try {
    ...
    detectColumns(firstRow) {
        ...
        for (const key of requiredColumns) {
            const possibleHeaders = F8949Parser.EXPECTED_HEADERS[key];
    ...
    combineForms(shortTermForms, longTermForms) {
...
F8949Parser.MAX_ROWS_PER_FORM = 14;
F8949Parser.ONE_YEAR_MS = 365 * 24 * 60 * 60 * 1000;
// Expected column headers (case-insensitive matching)
F8949Parser.EXPECTED_HEADERS = {
